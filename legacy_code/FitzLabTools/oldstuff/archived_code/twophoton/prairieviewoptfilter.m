...
im = previewprairieview(slowdir,2,1,channel);
im_outline = zeros(size(im)); im_outline(10:end-10,10:end-10) = 1;
pixelinds = {find(im_outline==1)};
data = readprairieviewdata(slowdir,[-Inf Inf],pixelinds,21,channel);
...
while i<length(frameTimes),
	...
	data = readprairieviewdata(fastdir,frameTimes([start stop]),pixelinds,21,channel);
