function [result,indimages] = prairieviewsinglecondition(dirname, channel, thetrials, timeint, sponttimeint, plotit,name)

%  PRAIRIEVIEWIMAGEMATH - Simple image math
%
%  [RESULT,INDIMAGES] = PRAIRIEVIEWSINGLECONDITION(DIRNAME,CHANNEL,
...
im = previewprairieview(dirname,2,1,channel);
...
for i=1:numStims(s.stimscript),
	...
	data = readprairieviewdata(dirname,myintervals-starttime,pixinds,21,channel);
	datasp = readprairieviewdata(dirname,myspintervals-starttime,pixinds,21,channel);
	indimages{i} = data{1,1} - datasp{1,1};
