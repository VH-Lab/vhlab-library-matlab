function [resp] = prairieviewtuningcurve(dirname, channel, paramname, pixels, plotit, names, trials, tint,sptint, blst, basemeth)
...
%
%  DIRNAME is the directory where the data resides.  CHANNEL is the
...
for p=1:size(data,2),
    ...
    if baselinemethod==3,
        ...
        else, baseline = repmat(meanforbaselines(p),1,(size(interval,1)+size(spinterval,1)));
        end;
    else,
