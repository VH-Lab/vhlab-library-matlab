function [mti2,starttime] = fitzcorrectmti(mti, filename,globaltime)

% FITZCORRECTMTI - Correct NewStim MTI based on recorded times
%
% [MTI2,STARTTIME] = FITZCORRECTMTI(MTI, STIMTIMEFILE,[GLOBALTIME])
%
...
if length(mti)>1, % if more than one stim, then use stim start times to calc
	for i=1:length(mti),
		...
		mti{i}.startStopTimes = [stimdata(2)-5 stimdata(2) stimdata(2)+5 stimdata(2)+5];
		mti{i}.frameTimes = stimdata(3:end);
		mti{i}.stimid = stimdata(1);
