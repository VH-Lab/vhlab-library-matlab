...
ma=mustask;
re=replaceexisting;
lsu=leaveselfunlabeled;
lou=leaveothersunlabeled;
if length(desc)==1,
	if strcmp(desc{1},param),
		is_type=1;
	end;
elseif length(desc)==2,
	if strcmp(desc{1},param) & strcmp(desc{2},'nCycles'),
		is_type = 1;
	end;
end;
...
switch type,
	...
	case 'Color exchange test',
		...
		if testgood,
			...
			chs = std(chromhigh);
			cls = std(chromlow);
			if all([chs([1 3]) cls([1 3])]<(1e-3+[0 0 0 0]))&chs(2)>0&cls(2)>0,
