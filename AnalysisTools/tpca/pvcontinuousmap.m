function [im,scalebaraxes,scalebarimage]=pvcontinuousmap(dirname,channel,cells,cellnames,indexassoc,funcstring,steps,plotit, varargin)
...
if nargin==0,  % it is the buttondownfcn
	...
	if v<50,
        ...
        figure;
        myargs = {};
        plottpresponse(ud.cells{i},ud.cellnames{i},ud.indexassoc,1,1,ud.varargin{:});
	end;
...
if vectorgraphics==1, plotit=1; end;

if vectorgraphics==1,
    figure;
    image(cat(3,im1,im2,im3));
    hold on;
end;
...
if plotit,
	...
	myud = struct('pts',pts,'inclpts',inclpts,'cells',{cells(inclpts)},'cellnames',{cellnames(inclpts)},'indexassoc',indexassoc,'imagesize',[size(im,1) size(im,2)],'values',values);
