function output = lfpcomp_meanresp(L, stdintervals, intvals, intspvals,name, B, A)
...
%
%  Computes the mean response for various time intervals.
%
...
%
%    tm_NAME{i}   A list of each trial's response.   The string NAME{i} is
...
for i=2:size(L,2),
	...
	for p=1:length(name),
		T{p}(i-1) = mean((d(tstart(p):tend(p))));
		Ts{p}(i-1) = mean((d(ts0(p):ts1(p))));
	end;
end;

output = [];
for p=1:length(name),
	output = setfield(output,['tm_' name{p} ], T{p});
	output = setfield(output,['tms_' name{p} ], Ts{p});
end;
