...
% LFPCOMP_INTEG - Integrates absolute value over an interval for LFP/CSD data
%
%  OUTPUT = LFPCOMP_INTEG(L, [NOT_USED], TIMEINTERVALS, SPONTINTERVALS, NAMES, B, A)
%
%  Computes the absolute integral for various time intervals.
%
...
for i=2:size(L,2),
	...
	for p=1:length(name),
		T{p}(i-1) = mean(std(d(tstart(p):tend(p))));
		Ts{p}(i-1) = mean(std(d(ts0(p):ts1(p))));
	end;
end;
