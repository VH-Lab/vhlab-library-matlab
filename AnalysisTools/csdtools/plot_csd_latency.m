...
%   a struct entries called 't_030_050' and 't_050_070'.
...
for t=start:step:stop,
	name1{end+1} = sprintf('t_%.3d_%.3d',round(t*1000),round(1000*(t+wsz)));
	name2{end+1} = sprintf('ts_%.3d_%.3d',round(t*1000),round(1000*(t+wsz)));
end;
...
for i=1:length(h),
	...
	plot(xdata, (stats<0.01)*max(yaxis), 'o');
	set(gca,'ytick',[]);
end;
