function [outputs] = intrinsic_mouse_md_sf_analysis2_steveedit(base_directory, animal_number, varargin)
%
...
[base_directory filesep 'sf_responses.txt']
...
output_blank = emptystruct('condition_name','roi_name', 'sfs','sf_responses','sf_responses_raw','sf_reference_region','blank_response','blank_response_raw','line','log','logthreshold','base_directory','dirname','Cgaussfit','Cdogfit');  % chelsea add fields here
...
for i=1:length(names),
	for j=1:length(roi_names),
        ...
        output_here.sf_responses = intrinsic_roi_responses([base_directory filesep dirname],roi_indexes);
        output_here.sf_responses_raw = intrinsic_roi_responses([base_directory filesep dirname],roi_indexes);
        output_here.sf_reference_region = intrinsic_roi_responses([base_directory filesep dirname],background_roi_indexes);
...
if 0,        
		for n=1:numStims(stimlist.saveScript),
			...
			elseif isfield(p,'isblank'), % is a blank stimulus
				...
				output_here.blank_response_raw = Response_sign * ( mean(s(roi_indexes))  );
			end;
		end;
end
		...
		[output_here.line.slope,output_here.line.y_intercept] = quickregression(output_here.sf_responses(1,1:end-1)', output_here.sf_responses(2,1:end-1)',0.05); % (:) for columns
		...
		linepowerthresholdfit(-log10(output_here.sf_responses(1,1:end-1)'),output_here.sf_responses(2,1:end-1)','exponent_start',1,'exponent_range',[1 1]);

		[output_here.log.slope,output_here.log.y_intercept] = quickregression(log10(output_here.sf_responses(1,1:end-1)'), output_here.sf_responses(2,1:end-1)',0.05); % (:) for columns
		output_here.log.x_intercept = -output_here.log.y_intercept / output_here.log.slope;
 
		[output_here.Cgaussfit.Rsp, output_here.Cgaussfit.Rp, output_here.Cgaussfit.P, output_here.Cgaussfit.sigm, output_here.Cgaussfit.FITCURVE, output_here.Cgaussfit.ERR] = Chelsea_gaussfit(output_here.sf_responses(1,1:end-1)', output_here.sf_responses(2,1:end-1)')
