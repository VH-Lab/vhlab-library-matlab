...
function [freqOut responseOutput lineOutput] = responsePlot_raw(base_directory, animal_number, fighand, color, norm, varargin)
...
for n=1:2
    ...
    for i=2,
        ...
        BlankResponses = A(2,9)
        A(2,:)= A(2,:)-BlankResponses
        x_sfs = A(1,:)
        y_response=A(2,:)
        errormonkey = A(4,:)
        if norm == 1
...
%         errorbar(x_sfs,y_response, errorToPlot)
...
%     [slope,offset,t,exponent,thefit]=linepowerthresholdfit(x,y,'exponent_start',1,'exponent_range',[1 1]);
  ...
  y_line = slope*rectify(-x_sfs2-threshold)
    ...
    % Save the figure
    name = [base_directory(end-9:end),'_',num2str(animal_number),'.png']
