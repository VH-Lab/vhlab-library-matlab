...
for k=1:3,
    ...
    for i=1:20,
        ...
        oi_t=compute_orientationindex(output.measured_angles,output.orimn);
	di_t=compute_directionindex(output.measured_angles,output.orimn);
        for j=1:5,  
    ...
    di_theory_total(k,:) = mean(di_theory,2)';
    std_2(k,:) = std(dis,0,2)';
