...
%disp('drawing raster'),
   % 2023-01-04 SDV updating for readability and adding comments from the ... 1999/2000 code?
...
if ~isempty(w),
	...
	mi = 1;
	ni = 1;
	ohno = length(n)+1-ni; % these variable names seem to be partially inspired by Monty Python

	for i=1:length(I.triggers),  % for each raster that we have do the following
		if ni>N, % shouldn't happen
			warning('Ni! Ni!'); % clearly inspired by Monty Python, what was I thinking in 2000?
		end; 
		if p.fracpsth>0, % should we plot the psth? If so, do it
			%disp('plotting psth');
			f = 1;
			if p.normpsth,
			...
			if p.psthmode==0, % bars
				...
				h = plot(ra.internals.bins{i},ra.internals.counts{i}*f);
			end;
			...
			if p.fracpsth==1,
				title(I.condnames{i},'interp','none');
			end;
		end;

		if p.fracpsth<1,  % do we have any of the raw raster to plot? if so, do it
		...
		ohno=length(n)+1-ni;
	end; % stimulus list, that is, for i=1:length(I.triggers)
