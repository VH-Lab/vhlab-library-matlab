...
% DRAWCUBE - Draws reverse correlation in 3d cube
%
%   [RC_AVG, XSTEPS, YSTEPS, AX, S1, S2, S3, YEL1, YEL2]=DRAWCUBE(RC, DRAWRECT)
%
%   Plots the reverse correlation response in a cube.
%
%   RC_AVG - The responses for the selected time bin, potentially averaged over multiple
%            stimulus presentations or cells.
%   AX - The axes that are plotted
%   S1-3 - Handles to the 3 surfaces that are plotted on the floor, side wall, and back wall
%   YEL1-2 - Handles to sets of yellow lines that indicate the currently selected area

w  = location(rc); ax = axes('units',w.units,'Position',drawrect);
...
if p.normalize==0,
	rc_avg=p.gain*(c.reverse_corr.rc_avg(p.datatoview(1),:,:,:,:)-p.feamean)+...
	         p.immean;
	rc_avg(find(rc_avg>255)) = 255;
	rc_avg(find(rc_avg<0)) = 0;
	rc_avg_t = mean(rc_avg(1,:,:,:,:),2);
	rc_avg_x = mean(rc_avg(1,:,:,:,:),4);
	rc_avg_y = mean(rc_avg(1,:,:,:,:),3);
	rc_avg_s = (rc_avg(1,p.datatoview(2),:,:,:));
	xx = size(rc_avg_t,4); yy = size(rc_avg_t,3);
	tt = size(rc_avg_x,2);
	p2 = getparameters(I.stimtime(1).stim);
	rect = p2.rect;
	tsteps2 = p.interval(1):p.timeres:p.interval(2);
	tint=[tsteps2(1) tsteps2(end)];
	if length(tsteps2)==1, tsteps2= [p.interval(1) p.interval(2)]; end;
        % these lines are magic, but seem to produce the index values of the selected time bin?
	rc_reshape = reshape(fix(rc_avg_s),xx*yy,3); sz_ = size(rc_reshape,1);
	rctreshape = reshape(fix(rc_avg_t),xx*yy,3); szt = size(rctreshape,1);
	rcxreshape = reshape(fix(rc_avg_x),yy*tt,3); szx = size(rcxreshape,1);
	rcyreshape = reshape(fix(rc_avg_y),xx*tt,3); szy = size(rcyreshape,1);
	[ctab,dum,inds]=unique([rc_reshape;rctreshape;rcxreshape;rcyreshape;],'rows');
	colormap(ctab/255);
...
elseif p.normalize==1, % normalize by difference from mean assuming two choices
	...
	z1 = norm(p2.values(1,:)); z2 = norm(p2.values(2,:)); % pick brighter stim
	if z1>=z2, z=0; else, z=1; end;
	meanvec = mean(p2.values);
	step = (p2.values(z+1,:)-meanvec)/sum(c.reverse_corr.norms);
	z = zeros(size(rc_avg)); z2=z;
	z(:,:,:,:,1) = meanvec(1); z2(:,:,:,:,1) = step(1); 
	z(:,:,:,:,2) = meanvec(2); z2(:,:,:,:,2) = step(2);
	z(:,:,:,:,3) = meanvec(3); z2(:,:,:,:,3) = step(3);
	...
	tsteps2 = p.interval(1):p.timeres:p.interval(2); tint=[tsteps2(1) tsteps2(end)];
	if length(tsteps2)==1, tsteps2= [p.interval(1) p.interval(2)]; end;
	rc_reshape = reshape(fix(rc_avg_s),xx*yy,3); sz_ = size(rc_reshape,1);
	rctreshape = reshape(fix(rc_avg_t),xx*yy,3); szt = size(rctreshape,1);
	rcxreshape = reshape(fix(rc_avg_x),yy*tt,3); szx = size(rcxreshape,1);
