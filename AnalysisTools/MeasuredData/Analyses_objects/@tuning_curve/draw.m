...
%disp('drawing tuning_curve'),
w = location(tc); p = getparameters(tc); I = getinputs(tc); c = getoutput(tc);
...
if ~isempty(w),
   ...
   elseif p.showrast,
      ...
      rct2=grect2local([0.1300 0.5 0.7750 0.39],w.units,w.rect,w.figure);
   else, rct=grect2local([0.1300 0.1100 0.7750 0.8150],w.units,w.rect,w.figure);
   end;
