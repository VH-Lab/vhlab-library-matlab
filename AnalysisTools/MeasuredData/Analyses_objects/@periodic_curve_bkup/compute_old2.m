  ...
  for z=1:length(vars),  % looping over second parameter
    ...
    for j=1:length(vals{z}), % looping over first parameter
       ...
       for nn=1:nCyc,
          ...
          cf1f0mean(nn)=abs(mean(divide_nozero((c.fftvals{nn}(loc1,:)),(c.fftvals{nn}(1,:))-avg_rate)));
          ...
          cf2f1mean(nn)=abs(mean(divide_nozero((c.fftvals{nn}(loc2,:)),(c.fftvals{nn}(loc1,:)))));
    ...
    for nn=1:length(stims),
          ...
          cf1f0meanc(nn)=abs(mean(divide_nozero((c1.fftvals{nn}(loc1c,:)),(c1.fftvals{nn}(1,:))-avg_rate)));
          ...
          cf2f1meanc(nn)=abs(mean(divide_nozero((c1.fftvals{nn}(loc2c,:)),(c1.fftvals{nn}(loc1c,:)))));
