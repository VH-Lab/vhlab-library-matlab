...
%  See also:  ANALYSIS_GENERIC/compute, PERIODIC_CURVE
  ...
  % compute_old was made 2002-04-15; it calculated all f1's, f0's, etc., by
  % using the mean of the absolute values of the ffts for individual trials.
  % I now think it is better to take the abs of the mean.
...
if ~isempty(pc.internals.oldparams), 
 ...
 if length(vars)==0, vars={[]}; end;
  ...
  RAparams.psthmode = 0; RAparams.showfrac = 1;
 ...
 % general parameters for rasters
  ...
  for z=1:length(vars),  % looping over second parameter
    ...
    for j=1:length(vals{z}), % looping over first parameter
       ...
       for nn=1:nCyc,
          nn,
          cf1mean(nn)=mean(abs(c.fftvals{nn}(loc1,:))); cf1stddev(nn)=std(abs(c.fftvals{nn}(loc1,:)));
          cf1stderr(nn)=cf1stddev(nn)/sqrt(length(stimlist));
          cf2mean(nn)=mean(abs(c.fftvals{nn}(loc2,:))); cf2stddev(nn)=std(abs(c.fftvals{nn}(loc2,:)));
          cf2stderr(nn)=cf2stddev(nn)/sqrt(length(stimlist));
          cf1f0mean(nn)=mean(divide_nozero(abs(c.fftvals{nn}(loc1,:)),abs(c.fftvals{nn}(1,:))-avg_rate));
          cf1f0stddev(nn)=std(divide_nozero(abs(c.fftvals{nn}(loc1,:)),abs(c.fftvals{nn}(1,:))-avg_rate));
          cf1f0stderr(nn)=cf1f0stddev(nn)/sqrt(length(stimlist));
          cf2f1mean(nn)=mean(divide_nozero(abs(c.fftvals{nn}(loc2,:)),abs(c.fftvals{nn}(loc1,:))));
          cf2f1stddev(nn)=std(divide_nozero(abs(c.fftvals{nn}(loc2,:)),abs(c.fftvals{nn}(loc1,:))));
    ...
    for nn=1:length(stims),
          ...
          cf1meanc(nn)=mean(abs(c1.fftvals{nn}(loc1c,:)));cf1stddevc(nn)=std(abs(c1.fftvals{nn}(loc1c,:)));
          cf1stderrc(nn)=cf1stddevc(nn)/sqrt(length(stims));
          cf2meanc(nn)=mean(abs(c1.fftvals{nn}(loc2c,:))); cf2stddevc(nn)=std(abs(c1.fftvals{nn}(loc2c,:)));
          cf2stderrc(nn)=cf2stddevc(nn)/sqrt(length(stims));
          cf1f0meanc(nn)=mean(divide_nozero(abs(c1.fftvals{nn}(loc1c,:)),abs(c1.fftvals{nn}(1,:))-avg_rate));
          cf1f0stddevc(nn)=std(divide_nozero(abs(c1.fftvals{nn}(loc1c,:)),abs(c1.fftvals{nn}(1,:))-avg_rate));
          cf1f0stderrc(nn)=cf1f0stddevc(nn)/sqrt(length(stims));
          cf2f1meanc(nn)=mean(divide_nozero(abs(c1.fftvals{nn}(loc2c,:)),abs(c1.fftvals{nn}(loc1c,:))));
          cf2f1stddevc(nn)=std(divide_nozero(abs(c1.fftvals{nn}(loc2c,:)),abs(c1.fftvals{nn}(loc1c,:))));
          ...
          cf0mean(nn)=mean(abs(c.fftvals{nn}(1,:)));cf0stddev(nn)=std(abs(c.fftvals{nn}(1,:)));
