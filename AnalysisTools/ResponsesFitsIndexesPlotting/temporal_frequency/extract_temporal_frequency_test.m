function [output] = extract_temporal_frequency_test(cell, prefix, testname, color, useF0)
% Extract common orientation/direction index values from a cell from a given test
%
%  [OUTPUT] = EXTRACT_TEMPORAL_FREQUENCY_TEST(CELL, PREFIX, TESTNAME, COLOR [, USEF0])
%
...
%    *tf_pref -- the temporal frequency with the most firing
...
f1f0 = NaN; tf_pref = NaN; blank_rate = NaN; max_rate = NaN;
coef_var = NaN; dr = NaN; sig_vis = NaN; time = NaN; tf_response_curve = NaN;
...
assoc_names = {'TF visual response p','TF Blank Response','TF Pref', 'TF Response curve'};
...
fn = {'f1f0', 'tf_pref', 'sig_vis','blank_rate', 'tf_response_curve', ...
		'time',...
