function [se,si,le,li,re,ri,err,r2,fit] = color_fit_rect4_rods(Lc,Sc,Rc,data,SE0,SI0,LE0,LI0,RE0,RI0);
...
% initial conditions
xo = [SE0 SI0 LE0 LI0 RE0 RI0];
options= optimset('Display','off','MaxFunEvals',10000,'TolX',1e-6);
[x] = fminsearch(@(x) color_fit_rect4_rods_err(x,Lc,Sc,Rc,data),xo,options);
if abs(x(1))>abs(x(2)), se = x(1); si = x(2); le = x(3); li = x(4); re=x(5);ri=x(6);
else, se = x(2); si = x(1); le = x(4); li = x(3); re=x(6);ri=x(5);
end;
[err,fit] = color_fit_rect4_rods_err(x,Lc,Sc,Rc,data);
