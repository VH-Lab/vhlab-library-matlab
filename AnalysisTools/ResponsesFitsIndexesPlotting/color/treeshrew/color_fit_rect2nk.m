function [l,s,lc0,sc0,lcN,scN,err,r2,fit] = color_fit_rect2nk(Lc,Sc,data,L0,S0,Lc00,Sc00,Ln0,Sn0);
...
%    R(LC,SC) = RECTIFY(L*LC./(abs(LC)+Lc0)+S*SC./(abs(SC)+Sc0))
...
[x] = fminsearch(@(x) color_fit_rect2nk_err(x,Lc,Sc,data),xo,options);
...
[err,fit] = color_fit_rect2nk_err(x,Lc,Sc,data);
