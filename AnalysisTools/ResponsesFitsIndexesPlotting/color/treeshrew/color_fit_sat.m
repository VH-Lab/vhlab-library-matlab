function [l,s,cs,err,r2,fit] = color_fit_sat(Lc,Sc,data,L0,S0);
...
[x] = fminsearch(@(x) color_fit_sat_err(x,Lc,Sc,data),xo,options);
if x(1)<0, x(1:2) = -x(1:2); end;
l=x(1);s=x(2);
Csint = [0.5 1];
cs = Csint(1)+diff(Csint)./(1+abs(x(3)));
[err,fit] = color_fit_sat_err(x,Lc,Sc,data);
