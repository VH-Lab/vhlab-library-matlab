function output = extract_spatial_frequency_all(cells,cellnames,prefix,testnamelist)
% EXTRACT_SPATIAL_FREQUENCY_ALL - Extract orientation and direction index information for cells studied across time
%
%  OUTPUT = EXTRACT_SPATIAL_FREQUENCY_ALL(CELLS,CELLNAMES,PREFIX,TESTNAMELIST)
%
...
%
%  See also: READCELLSFROMEXPERIMENTLIST, LOADCELLLIST, EXTRACT_SPATIAL_FREQUENCY_TEST
%
...
for i=1:length(cells),
	...
	if ~isempty(newoutput.testname),
		...
		for n=1:max_repeats, 
			...
			if ~isempty(z),
				if n==1,
					indexes_here = extract_spatial_frequency_test(cells{i},prefix,[newoutput.testname int2str(n)]);
