...
for i=3:length(g),
        if (exist([g(i).name '/raw data/'])), cd([g(i).name '/raw data/']); end;
	pwd,
        if exist('records')
		disp('exploring...');
		...
		if (j<=length(r))&(r(j).ref==recparam.ref)& ...
			...
			elseif strcmp(r(j).type,'extracellular'),
				fileend = r(j).fname;
			...
			if (r(j).reps>3&algor_p.automatic==1)&~got_thresh, 
				fname=[ '/r' sprintf('%.3i',1) '_' fileend];
				...
				sd = sd * std(data),
				got_thresh = 1;
			elseif (algor_p.automatic==1)&~got_thresh,
				error(['spikedetect: threshold could not' ...	
