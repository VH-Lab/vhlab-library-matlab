function vhintan_spikesorting(varargin)
% VHINTAN_SPIKESORTING - A gui to guide a user through spikesorting multichannel LabView data
%
%   VHINTAN_SPIKESORTING('DS', DS);
%
...
switch command,
	...
	case 'NameRefList',
		vhintan_spikesorting('fig',fig,'command','EnableDisable');
	...
	case 'ThresholdsBt',
		...
		vhintan_setthresholds_gui('ds',ud.ds,prefs{:});
	case 'AutoThresholdsBt',
		prefs = struct2namevaluepair(ud.spikesortingprefs);
		vhintan_autothreshold_all(ud.ds,prefs{:});
		msgbox('Auto thresholding completed successfully.');
	case 'ExtractSelectBt',
		prefs = struct2namevaluepair(ud.spikesortingprefs);
		vhintan_extractselected(getpathname(ud.ds),prefs{:});
	case 'EnableDisable',
		v = get(findobj(fig,'tag','NameRefList'),'value');
		if isempty(v),
