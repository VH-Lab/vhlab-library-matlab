function [dirnames, status] = vhintan_getdirectorystatus(ds, optdirname)
% VHINTAN_GETDIRECTORYSTATUS - Get the status of directories that have Multichannel LabView data
%
%    [DIRS,STATUS] = VHINTAN_GETDIRECTORYSTATUS(DS, [DIRNAME])
%
...
if nargin>=2,
	...
	for i=1:length(total_dirs),
		d = exist(vhintan_getdirfilename([pn filesep total_dirs{i}]));
		if d==2,
			incl(end+1) = i;
		end;
	end;
