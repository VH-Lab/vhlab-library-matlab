...
%
%   THRESHOLDS = SETTHRESHOLDS_GUI('datafile',FILENAME,'header',HEADER,...
...
switch command,
	...
	case 'UpdateBt',
		...
		[newdirlist, newdirinfo] = vhNDISpikeSorter.getdirectorystatus(ud.ds);
			% this is the place to ask the user if they want to apply thresholds
		labels = vhNDISpikeSorter.directorystatus2labels(newdirlist,newdirinfo);
	...
	case 'UpdateChannelMenu',
		...
		channelmenustring = vhNDISpikeSorter.thresholdchannellabels(ud.dirinfo(dirvalue));
		...
		if ud.channelhold, % we're just updating the popup labels, we don't want to update the values
			...
			set(fig,'userdata',ud);
			vhNDISpikeSorter.setthresholds_gui('command','ChannelPopup','fig',fig);
		end;
	case 'DirectoryPopup',
		dirmenu_currentvalue = get(findobj(fig,'tag','DirectoryPopup'),'value');
		...
		set(fig,'userdata',ud);
		if b,
			vhNDISpikeSorter.setthresholds_gui('command','UpdateChannelMenu','fig',fig); % will call ChannelPopup, which calls Load
		end;
	case 'ChannelPopup',
	...
	case 'Load',
		...
		header_filename = vhNDISpikeSorter.getdirfilename(pathname);
	...
	case 'ExtractBt',
		...
		if strcmp(upper(ans),'YES'),
			vhNDISpikeSorter.extractwaveforms([getpathname(ud.ds) filesep ud.dirlist{dirmenu_currentvalue}], ud.SAMPLES, ud.REFRACTORY_SAMPLES, 'MEDIAN_FILTER_ACROSS_CHANNELS',ud.MEDIAN_FILTER_ACROSS_CHANNELS);
			vhNDISpikeSorter.sync2spike2([getpathname(ud.ds) filesep ud.dirlist{dirmenu_currentvalue}]);
		end;
		ud.channelhold = 1;
		set(fig,'userdata',ud);
		vhNDISpikeSorter.setthresholds_gui('fig',fig,'command','UpdateChannelMenu');
