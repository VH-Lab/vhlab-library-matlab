function vhspike2_extractselected(dirname, varargin)
% VHSPIKE2_EXTRACTSELECTED - Extract spikes from user-specified directories for a given experiment
%
%    VHSPIKE2_EXTRACTSELECTED(DIRNAME, ...)
%
...
%                                     :     (uses vhspike2_filtermap.txt -- see help for this file)
...
if ok,
    ...
    if usep,
        ...
        parfor t=1:length(s),
            vhspike2_extractwaveforms([getpathname(ds) filesep T{s(t)}], SAMPLES, REFRACTORY_PERIOD_SAMPLES, ...
            	'MEDIAN_FILTER_ACROSS_CHANNELS',MEDIAN_FILTER_ACROSS_CHANNELS);
        	vhspike2_sync2spike2([getpathname(ds) filesep T{s(t)}]);
        end;
