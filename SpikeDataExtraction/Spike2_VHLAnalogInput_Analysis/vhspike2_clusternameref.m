function vhspike2_clusternameref(ds, name, ref)
% VHSPIKE2_CLUSTERNAMEREF - Cluster a name/reference pair collected using Spike2
%
%   VHSPIKE2_CLUSTERNAMEREF(DS, NAME, REF)
%
...
%  text files called 'vhspike2_st_name_ref_00M.txt', where M is the cluster number.
...
for i=1:length(dirlist),
	...
	[spikewaveforms, header] = readvhlspikewaveformfile([dirname filesep 'vhspike2_spikewaveforms_' int2str(channelgroupingnum) '.vsw']);
    try,
	...
	fid = fopen([dirname filesep 'vhspike2_spiketimes_' int2str(channelgroupingnum) '.vst'],'r','b');
    if fid<0, error(['Could not open file ' [dirname filesep 'vhspike2_spiketimes_' int2str(channelgroupingnum) '.vst'] '.']); end;
...
fname_prefix = ['vhspike2_st_' name '_' int2str(ref) '_'];
infofname_prefix = ['vhspike2_ci_' name '_' int2str(ref) '_'];
