function varargout = vhspike2_setthresholds_gui(varargin)
% VHSPIKE2_SETTHRESHOLDS_GUI - Select thresholds based on multichannel data
%
%   THRESHOLDS = VHSPIKE2_SETTHRESHOLDS_GUI('datafile',FILENAME,'header',HEADER,...
...
%   (as returned by READVHSPIKE2HEADERFILE), and CHANNEL_LIST should be the list of
%   channels to examine.
%
%   THRESHOLDS is a structure that can be written to the file vhspike2_threshold.txt
%   (see 'help vhspike2_threshold')
%  
...
switch command,
	case 'Main',
		set(fig,'userdata',ud);
		vhspike2_setthresholds_gui('command','NewWindow','fig',fig);
		vhspike2_setthresholds_gui('command','UpdateBt','fig',fig);
		%uiwait(fig);
		%ud = get(fig,'userdata');
		%if nargout>=1,
	...
	case 'NewWindow',
		...
		set(fig,'position',[50 50 right top],'tag','vhspike2_setthresholds_gui');
	...
	case 'UpdateBt',
		...
		[newdirlist, newdirinfo] = vhspike2_getdirectorystatus(ud.ds);
			% this is the place to ask the user if they want to apply thresholds
		labels = vhspike2_directorystatus2labels(newdirlist,newdirinfo);

		set(findobj(fig,'tag','DirectoryPopup'),'string',labels,'value',1);
		ud.dirlist = newdirlist;
		ud.dirinfo = newdirinfo;
		set(fig,'userdata',ud);
		vhspike2_setthresholds_gui('command','UpdateChannelMenu','fig',fig);
	case 'CopyThresholdsBt',
		dirmenu_currentvalue = get(findobj(fig,'tag','DirectoryPopup'),'value');
		outputstr = vhspike2_copythresholds(ud.ds,ud.dirlist{dirmenu_currentvalue});
	...
	case 'UpdateChannelMenu',
		...
		channelmenustring = vhspike2_thresholdchannellabels(ud.dirinfo(dirvalue));
		...
		if ud.channelhold, % we're just updating the popup labels, we don't want to update the values
			...
			set(fig,'userdata',ud);
			vhspike2_setthresholds_gui('command','ChannelPopup','fig',fig);
		end;
	case 'DirectoryPopup',
		dirmenu_currentvalue = get(findobj(fig,'tag','DirectoryPopup'),'value');
		...
		set(fig,'userdata',ud);
		if b,
			vhspike2_setthresholds_gui('command','UpdateChannelMenu','fig',fig); % will call ChannelPopup, which calls Load
		end;
	case 'ChannelPopup',
		...
		set(fig,'userdata',ud);
		if b,
			vhspike2_setthresholds_gui('command','Load','fig',fig);
		end;
	case 'DrawAxes',
		...
		vhspike2_setthresholds_gui('command','DrawThresholds','fig',fig);
	...
	case 'DrawThresholds',  % draw threshold AND update spike axes
		...
		channels_here = ud.dirinfo(dirmenu_currentvalue).vhspike2_filtermap_values(channelmenu_currentvalue).channel_list;
		thresholds = ud.dirinfo(dirmenu_currentvalue).vhspike2_thresholds_values;
	...
	case 'VoltageAxesClick',  % this sets the threshold for a given channel
		...
		% now update the vhspike2_thresholds_value and save it
		...
		existing_thresholds = ud.dirinfo(dirmenu_currentvalue).vhspike2_thresholds_values;
		if isempty(existing_thresholds),
			...
			elseif length(z)==1,
				...
				error(['More than one threshold for channel ' int2str(channel_value) ' in directory ' ud.dirlist{dirmenu_currentvalue}  ', file vhspike2_thresholds.txt.']);
			end;
			[sorted_values,sorted_indexes] = sort([existing_thresholds.channel]);
			existing_thresholds = existing_thresholds(sorted_indexes);
		end;
		ud.dirinfo(dirmenu_currentvalue).vhspike2_thresholds_values = existing_thresholds;

		% at this point, our data structure is updated, but we must save to disk
		saveStructArray([getpathname(ud.ds) filesep ud.dirlist{dirmenu_currentvalue} filesep 'vhspike2_thresholds.txt'],ud.dirinfo(dirmenu_currentvalue).vhspike2_thresholds_values);

		% now re-draw only the channel that was modified
		ud.threshold_update_list = axnumber;
		ud.channelhold = 1;
		set(fig,'userdata',ud);
		vhspike2_setthresholds_gui('fig',fig,'command','UpdateChannelMenu');
		vhspike2_setthresholds_gui('fig',fig,'command','DrawThresholds');
	case 'Load',
		channelmenu_currentvalue = get(findobj(fig,'tag','ChannelPopup'),'value');
		dirmenu_currentvalue = get(findobj(fig,'tag','DirectoryPopup'),'value');
		channel_list =ud.dirinfo(dirmenu_currentvalue).vhspike2_filtermap_values(channelmenu_currentvalue).channel_list;
		...
		set(fig,'userdata',ud);
		vhspike2_setthresholds_gui('fig',fig,'command','DrawAxes');
	case {'MatchXYAxisBt'},
		ax = gca;
		A = axis;
	...
	case {'StartEdit','DurationEdit'}
		...
		if newstartvalue~=ud.start | newdurationvalue~=ud.windowsize,
			...
			set(fig,'userdata',ud);
			vhspike2_setthresholds_gui('fig',fig,'command','Load');
		end;
	case 'DoneBt',
		b = 1;
		if b,
	...
	case 'ExtractBt',
		dirmenu_currentvalue = get(findobj(fig,'tag','DirectoryPopup'),'value');
		ans = 'Yes';
		if ud.dirinfo(dirmenu_currentvalue).vhspike2_thresholds~=1,
		...
		if strcmp(upper(ans),'YES'),
			vhspike2_extractwaveforms([getpathname(ud.ds) filesep ud.dirlist{dirmenu_currentvalue}], ud.SAMPLES, ud.REFRACTORY_SAMPLES, 'MEDIAN_FILTER_ACROSS_CHANNELS',ud.MEDIAN_FILTER_ACROSS_CHANNELS);
			vhspike2_sync2spike2([getpathname(ud.ds) filesep ud.dirlist{dirmenu_currentvalue}]);
		end;
		ud.channelhold = 1;
		set(fig,'userdata',ud);
		vhspike2_setthresholds_gui('fig',fig,'command','UpdateChannelMenu');
