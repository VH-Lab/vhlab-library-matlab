function vhlv_autothreshold_all(ds, varargin)
% VHLV_AUTOTHRESHOLDALL - Perform automatic spike threshold determination on all records
%
%   VHLV_AUTOTHREHSOLD_ALL(DS, ...)
%
...
[dirs,status] = vhlv_getdirectorystatus(ds);
...
vhlv_autothreshold_dir([getpathname(ds) filesep dirs{1}],'sigma',sigma,'pretime',pretime,'usemedian',usemedian,...
	'MEDIAN_FILTER_ACROSS_CHANNELS',MEDIAN_FILTER_ACROSS_CHANNELS);

 % step 3 - copy these to subsequent directories

vhlv_copythresholds(ds,dirs{1});
