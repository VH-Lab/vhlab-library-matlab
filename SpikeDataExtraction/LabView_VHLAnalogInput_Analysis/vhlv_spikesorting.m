function vhlv_spikesorting(varargin)
% VHLV_SPIKESORTING - A gui to guide a user through spikesorting multichannel LabView data
%
%   VHLV_SPIKESORTING('DS', DS);
%
...
ds = [];               % dirstruct
windowlabel = 'VHLV Spike sorting';
...
if isempty(fig),
	z = findobj(allchild(0),'flat','tag','vhlv_spikesorting');
	if isempty(z),
		fig = figure('name','VHLV Spike Sorting','NumberTitle','off'); % we need to make a new figure
	else,
		fig = z;
		figure(fig);
		vhlv_spikesorting('fig',fig,'command','UpdateNameRefList');
...
switch command,
	case 'Main',
		set(fig,'userdata',ud);
		vhlv_spikesorting('command','NewWindow','fig',fig);
		vhlv_spikesorting('fig',fig,'command','UpdateNameRefList');
	case 'NewWindow',
        ...
        set(fig,'position',[50 50 right top],'tag','vhlv_spikesorting');
	...
	case 'UpdateBt',
		vhlv_spikesorting('fig',fig,'command','UpdateNameRefList');
	case 'ImportBt',
		vhlv_importcells(ud.ds);
	case 'ClusterBt',
		v = get(findobj(fig,'tag','NameRefList'),'value');
		for i=1:length(v),
			vhlv_clusternameref(ud.ds,ud.nr(v(i)).name,ud.nr(v(i)).ref);
		end;
	case 'NameRefList',
		vhlv_spikesorting('fig',fig,'command','EnableDisable');
	...
	case 'ThresholdsBt',
		...
		vhlv_setthresholds_gui('ds',ud.ds,prefs{:});
	case 'AutoThresholdsBt',
		prefs = struct2namevaluepair(ud.spikesortingprefs);
		vhlv_autothreshold_all(ud.ds,prefs{:});
		msgbox('Auto thresholding completed successfully.');
	case 'ExtractSelectBt',
		prefs = struct2namevaluepair(ud.spikesortingprefs);
		vhlv_extractselected(getpathname(ud.ds),prefs{:});
	case 'EnableDisable',
		v = get(findobj(fig,'tag','NameRefList'),'value');
		if isempty(v),
	...
	case 'PreferencesBt',
		name = 'VHLV Spikesorting preferences';
