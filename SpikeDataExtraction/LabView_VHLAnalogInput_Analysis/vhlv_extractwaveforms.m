function [wavetimes, t] = extractvhlvwaveforms(dirname, samples, refractory_period_samples, varargin)
% VHLV_EXTRACTVHLVWAVEFORMS - Extract spike waveforms from VH lab multichannel labview recordings
%
%     VHLV_EXTRACTVHLVWAVEFORMS(DIRNAME, [S0 S1], REFRACTORY_PERIOD_SAMPLES, ...)
%
...
%  vhlv_filtermap.txt that describes which channels should be grouped together
%  for the purpose of filtering (see help vhlv_filtermap).
%
%  It is also assumed that there is a channel grouping file 
%  vhlv_channelgrouping.txt, that indicates which channels should be grouped together
...
%  vhlv_thresholds.txt  that describes the thresholds to be used for
%  for detecting spikes on these electrode units (see help vhlv_thresholds).
%
%  The waveforms are saved to files lv_spikewaveforms_N.vsw, located
...
%  in the binary file lv_spiketimes_N.vsw (in float64 format).
%
%  EXTRACTVHLVWAVEFORMS uses a number of parameters for reading the spikes.
...
t = [samples(1):samples(2)]/header.SamplingRate;
...
threshold_filename = [dirname filesep 'vhlv_thresholds.txt'];
...
for k=1:length(channelgrouping),
	changrouping_wavefname{k} = [dirname filesep 'lv_spikewaveforms_' int2str(k) '.vsw'];
...
for i=1:length(filtermap),
	...
	while (~end_of_file_reached),
		...
		end_time = start_time + READSIZE;
			% read the data
		[T,D,tot_sam,tot_time] = readvhlvdatafile(data_filename,header,filtermap(i).channel_list,start_time,end_time);

		if abs(length(T) - ((end_time - start_time) * header.SamplingRate + 1))>0.1, % | T(end)>100,
		...
		for k=1:length(channelgrouping),
			...
			% now that we have the locations, let's read the waveforms
				...
				wavetimes{k} = cat(1,wavetimes{k},T(my_locations)');
				...
				if changrouping_created(k)==0,
					...
					myp.comment = dirname;
					myp.samplingrate = header.SamplingRate;
...
for k=1:length(channelgrouping), % reshape to have the right shape/order
	...
	fid=fopen([dirname filesep 'lv_spiketimes_' int2str(k) '.vst'],'w','b');
	if fid<0,
		error(['Could not open file ' dirname filesep 'lv_spiketimes_' int2str(k) '.vst']);
	end;
	fwrite(fid,wavetimes{k},'float64');
	fclose(fid);
	%save([dirname filesep 'lv_spiketimes_' int2str(k) '.mat'],'spiketimes');
end;
