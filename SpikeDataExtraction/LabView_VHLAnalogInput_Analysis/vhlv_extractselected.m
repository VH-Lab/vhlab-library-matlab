function vhlv_extractselected(dirname, varargin)
% VHLV_EXTRACTSELECTED - Extract spikes from user-specified directories for a given experiment
%
%    VHLV_EXTRACTSELECTED(DIRNAME, ...)
%
...
%                                     :     (uses vhlv_filtermap.txt -- see help for this file)
...
if ok,
    ...
    if usep,
        ...
        parfor t=1:length(s),
            vhlv_extractwaveforms([getpathname(ds) filesep T{s(t)}], SAMPLES, REFRACTORY_PERIOD_SAMPLES, ...
            	'MEDIAN_FILTER_ACROSS_CHANNELS',MEDIAN_FILTER_ACROSS_CHANNELS);
        	vhlv_sync2spike2([getpathname(ds) filesep T{s(t)}]);
        end;
