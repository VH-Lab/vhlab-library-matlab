function [status_str] = vhlv_copythresholds(ds, dirname)
% VHLV_COPYTHRESHOLDS - Copy thresholds from earlier records into later records
%
%   STATUS_STR = VHLV_COPYTHRESHOLDS(DS)
%
...
[dirlist,status] = vhlv_getdirectorystatus(ds);

thresh_done = [status.vhlv_thresholds];
...
vhlv_thresholds_filename = 'vhlv_thresholds.txt';

status_str = ['Thresholds of ' dirlist{lastdone} ' were applied to '];

for i=lastdone+1:length(dirlist),
	copyfile(fullfile(pathname,dirlist{lastdone},vhlv_thresholds_filename),...
		fullfile(pathname,dirlist{i},vhlv_thresholds_filename));
