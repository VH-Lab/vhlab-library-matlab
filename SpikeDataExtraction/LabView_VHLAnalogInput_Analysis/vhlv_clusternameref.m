function vhlv_clusternameref(ds, name, ref)
% VHLV_CLUSTERNAMEREF - Cluster a name/reference pair collected using LabView
%
%   VHLV_CLUSTERNAMEREF(DS, NAME, REF)
%
...
%  text files called 'lv_st_name_ref_00M.txt', where M is the cluster number.
...
for i=1:length(dirlist),
	...
	[spikewaveforms, header] = readvhlspikewaveformfile([dirname filesep 'lv_spikewaveforms_' int2str(channelgroupingnum) '.vsw']);
    try,
	...
	fid = fopen([dirname filesep 'lv_spiketimes_' int2str(channelgroupingnum) '.vst'],'r','b');
    if fid<0, error(['Could not open file ' [dirname filesep 'lv_spiketimes_' int2str(channelgroupingnum) '.vst'] '.']); end;
...
fname_prefix = ['lv_st_' name '_' int2str(ref) '_'];
infofname_prefix = ['lv_ci_' name '_' int2str(ref) '_'];
