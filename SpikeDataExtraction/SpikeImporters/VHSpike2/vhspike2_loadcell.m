...
% VHSPIKE2_LOADCELL - Load in a cell with a given name, reference, and index value
%
%  [SD, CELLNAME] = VHSPIKE2_LOADCELL(DS, NAME, REF, INDEX)
%
%  Loads the spike time data from Spike2 records into a CKSMULTIUNIT object
...
%  INDEX the cluster index to import
 ...
 % in the olden days, this function was called loadfitzcell.m
...
[A,I] = findassociate(oldcell,'loadfitzcelldata','loadfitzcell','');
...
for t=1:length(T),
	...
	filenum = 0;
	try,
		f = loadStructArray([pfix filesep T{t} filesep 'singleEC.txt']);
	catch,
		f = [];
	end;
	for j=1:length(f),
	...
	if C1,  % if isempty(ind)|isempty(lcfd(ind).interval)
		disp(['Attempting to import (Spike2): ' name ' | ' int2str(ref) ' ' int2str(index) ' ' T{t}]);
...
if ~isempty(ints),
	...
	sd = associate(sd,struct('type','loadfitzcelldata','owner','loadfitzcell','data',lfcd,'desc',''));
else, sd = [];
end;
...
if exist([dirname filesep 'stims.mat'])&exist([dirname filesep 'stimtimes.txt']),
	g = load([dirname filesep 'stims.mat'],'start','MTI2');
