function [sd,cellname] = vhintan_loadcell(ds, name, ref, index)
...
if isfield(oldcell,cellname),
  ...
  % if so, check most recent directory
	...
	% edit here -- probably always replace the vhintan_loadcell info

[A,I] = findassociate(oldcell,'vhintan_loadcelldata','vhintan_loadcell','');
...
if isgood&~isempty(ints),
	...
	sd = associate(sd,struct('type','vhintan_loadcelldata','owner','vhintan_loadcell','data',lfcd,'desc',''));
	sd = associate(sd,struct('type','vhintan_clusterinfo','owner','vhintan_loadcell','data',lfcd(good_example).clusterinfo,'desc','cluster info for a good directory'));
else, sd = [];
end;

function [spikes,istherespikerecord,clusterinfo]=getspikesfromdir(dirname,name,ref,index)

fname = [dirname filesep 'intan_st_' name '_' int2str(ref) '_' sprintf('%.3d',index) '.txt'];
try,
	clusterinfo = load([dirname filesep 'intan_ci_' name '_' int2str(ref) '_' sprintf('%.3d',index) '.mat'],'-mat');
...
if exist(fname)&~strcmp(clusterinfo.qualitylabel,'Not usable'),
	...
	shiftscale = load([dirname filesep 'vhintan_intan2spike2time.txt'],'-ascii');
