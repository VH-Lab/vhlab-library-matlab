...
%
%    VHSPIKE2_IMPORTCELLS(DS, NAMEREF)
%
...
for i=1:length(namerefs),
	...
	for t=1:length(T),
		D = dir([getpathname(ds) filesep T{t} filesep 'vhspike2_st_' namerefs(i).name '_' int2str(namerefs(i).ref) '_*.txt']);
		for d=1:length(D),
			index__here = sscanf(D(d).name,['vhspike2_st_' namerefs(i).name '_' int2str(namerefs(i).ref) '_%d.txt']);
