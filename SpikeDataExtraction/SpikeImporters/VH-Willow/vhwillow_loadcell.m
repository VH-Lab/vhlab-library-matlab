function [sd,cellname] = vhwillow_loadcell(ds, name, ref, index)
...
if isfield(oldcell,cellname),
  ...
  % if so, check most recent directory
	...
	% edit here -- probably always replace the vhwillow_loadcell info

[A,I] = findassociate(oldcell,'vhwillow_loadcelldata','vhwillow_loadcell','');
...
if isgood&~isempty(ints),
	...
	sd = associate(sd,struct('type','vhwillow_loadcelldata','owner','vhwillow_loadcell','data',lfcd,'desc',''));
	sd = associate(sd,struct('type','vhwillow_clusterinfo','owner','vhwillow_loadcell','data',lfcd(good_example).clusterinfo,'desc','cluster info for a good directory'));
else, sd = [];
end;

function [spikes,istherespikerecord,clusterinfo]=getspikesfromdir(dirname,name,ref,index)

fname = [dirname filesep 'willow_st_' name '_' int2str(ref) '_' sprintf('%.3d',index) '.txt'];
try,
	clusterinfo = load([dirname filesep 'willow_ci_' name '_' int2str(ref) '_' sprintf('%.3d',index) '.mat'],'-mat');
...
if exist(fname)&~strcmp(clusterinfo.qualitylabel,'Not usable'),
	...
	shiftscale = load([dirname filesep 'vhwillow_willow2spike2time.txt'],'-ascii');
