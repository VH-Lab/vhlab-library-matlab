function vhwillow_importcells(ds, nrs)
% VHWILLOW_IMPORTCELLS - Import cells from LabView that were spike-sorted with VH lab tools
%  
%    VHWILLOW_IMPORTCELLS(DS)
%
...
%
%    VHWILLOW_IMPORTCELLS(DS, NAMEREF)
%
...
for i=1:length(namerefs),
	...
	for t=1:length(T),
		D = dir([getpathname(ds) filesep T{t} filesep 'willow_st_' namerefs(i).name '_' int2str(namerefs(i).ref) '_*.txt']);
		for d=1:length(D),
			index__here = sscanf(D(d).name,['willow_st_' namerefs(i).name '_' int2str(namerefs(i).ref) '_%d.txt']);
	...
	for z=1:length(indlist),
		[mycell1,mycellname1] = vhwillow_loadcell(ds,namerefs(i).name,namerefs(i).ref,indlist(z));
