function [wavetimes, t] = extractvhwillowwaveforms(dirname, samples, refractory_period_samples, varargin)
% VHWILLOW_EXTRACTVHWILLOWWAVEFORMS - Extract spike waveforms from VH lab multichannel labview recordings
%
%     VHWILLOW_EXTRACTVHWILLOWWAVEFORMS(DIRNAME, [S0 S1], REFRACTORY_PERIOD_SAMPLES, ...)
%
...
%  vhwillow_filtermap.txt that describes which channels should be grouped together
%  for the purpose of filtering (see help vhwillow_filtermap).
%
%  It is also assumed that there is a channel grouping file 
%  vhwillow_channelgrouping.txt, that indicates which channels should be grouped together
%  for the purpose of identifying and distinguishing spike waveform shapes
%  (see help vhwillow_channelgrouping).
%
%  It is further assumed that there is a file called
%  vhwillow_thresholds.txt  that describes the thresholds to be used for
%  for detecting spikes on these electrode units (see help vhwillow_thresholds).
%
%  The waveforms are saved to files willow_spikewaveforms_N.vsw, located
...
%  in the binary file willow_spiketimes_N.vsw (in float64 format).
%
%  EXTRACTVHWILLOWWAVEFORMS uses a number of parameters for reading the spikes.
%  You can alter the values of these parameters by passing extra name/value
%  pairs to the function (such as 'READSIZE', 30).
%
%  Parameter, default                Comment:
%  READSIZE, 10                     : How big of data chucks should be read? (seconds)
...
filtermap_filename = [dirname filesep 'vhwillow_filtermap.txt'];
...
threshold_filename = [dirname filesep 'vhwillow_thresholds.txt'];
...
for k=1:length(channelgrouping),
	changrouping_wavefname{k} = [dirname filesep 'willow_spikewaveforms_' int2str(k) '.vsw'];
...
for i=1:length(filtermap),
	...
	t0__ = read_Willow_datafile(data_filename,header,'time',1,0,0);
	...
	while (~end_of_file_reached),
		...
		end_time = start_time + READSIZE;
			% read the data
		[D,tot_sam,tot_time] = read_Willow_datafile(data_filename,header,'amp',filtermap(i).channel_list,start_time,end_time);
...
for k=1:length(channelgrouping), % reshape to have the right shape/order
	...
	fid=fopen([dirname filesep 'willow_spiketimes_' int2str(k) '.vst'],'w','b');
	if fid<0,
		error(['Could not open file ' dirname filesep 'willow_spiketimes_' int2str(k) '.vst']);
	end;
	fwrite(fid,wavetimes{k},'float64');
	fclose(fid);
	%save([dirname filesep 'willow_spiketimes_' int2str(k) '.mat'],'spiketimes');
end;
