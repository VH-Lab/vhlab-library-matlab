function s=vhwillow_filtermap_default
% VHWILLOW_FILTERMAP_DEFAULT - Describes the vhwillow_filtermap.txt file for multichannel analysis
%
...
%   called "vhwillow_filtermap.txt" that describes how the channels should be 
...
current_channel = channelstart;

s = struct('channel_list',[1 2 3 4 5 6 7 8]);
s = s([]); % make it empty but with those fields

while (current_channel < channelstop)
	s(end+1) = struct('channel_list',(current_channel):(current_channel+channel_increment-1));
	current_channel = current_channel + channel_increment;
end;
