function vhwillow_autothreshold_all(ds, varargin)
% VHWILLOW_AUTOTHRESHOLDALL - Perform automatic spike threshold determination on all records
%
%   VHWILLOW_AUTOTHREHSOLD_ALL(DS, ...)
%
...
[dirs,status] = vhwillow_getdirectorystatus(ds);
...
vhwillow_autothreshold_dir([getpathname(ds) filesep dirs{1}],'sigma',sigma,'pretime',pretime,'usemedian',usemedian,...
	'MEDIAN_FILTER_ACROSS_CHANNELS',MEDIAN_FILTER_ACROSS_CHANNELS);

 % step 3 - copy these to subsequent directories

vhwillow_copythresholds(ds,dirs{1});
