function vhwillow_extractselected(dirname, varargin)
% VHWILLOW_EXTRACTSELECTED - Extract spikes from user-specified directories for a given experiment
%
%    VHWILLOW_EXTRACTSELECTED(DIRNAME, ...)
%
...
%                                     :     (uses vhwillow_filtermap.txt -- see help for this file)
...
if ok,
    ...
    if usep,
        ...
        parfor t=1:length(s),
            vhwillow_extractwaveforms([getpathname(ds) filesep T{s(t)}], SAMPLES, REFRACTORY_PERIOD_SAMPLES, ...
            	'MEDIAN_FILTER_ACROSS_CHANNELS',MEDIAN_FILTER_ACROSS_CHANNELS);
        	vhwillow_sync2spike2([getpathname(ds) filesep T{s(t)}]);
        end;
