function [status_str] = vhwillow_copythresholds(ds, dirname)
% VHWILLOW_COPYTHRESHOLDS - Copy thresholds from earlier records into later records
%
%   STATUS_STR = VHWILLOW_COPYTHRESHOLDS(DS)
%
...
[dirlist,status] = vhwillow_getdirectorystatus(ds);

thresh_done = [status.vhwillow_thresholds];
...
vhwillow_thresholds_filename = 'vhwillow_thresholds.txt';

status_str = ['Thresholds of ' dirlist{lastdone} ' were applied to '];

for i=lastdone+1:length(dirlist),
	copyfile(fullfile(pathname,dirlist{lastdone},vhwillow_thresholds_filename),...
		fullfile(pathname,dirlist{i},vhwillow_thresholds_filename));
