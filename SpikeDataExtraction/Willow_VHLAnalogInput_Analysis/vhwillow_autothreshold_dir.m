function output_str = vhwillow_autothreshold_dir(dirname, varargin)
% VHWILLOW_AUTOTHRESHOLD_DIR - Automatically determine spike threshold values for a directory
%
%  OUTPUT_STR = VHWILLOW_AUTOTHRESHOLD_DIR(DIRNAME, ...)
%
...
for i=1:length(filtermap), 
	[D,tot_sam,tot_time] = read_Willow_datafile(data_filename,header,'amp',filtermap(i).channel_list,start_time,stop_time);
	D = filtfilt(B,A,D);
...
saveStructArray([dirname filesep 'vhwillow_thresholds.txt'],threshold_struct,1);
